...
const FIRST = ['Sarah', 'Michael', 'Jennifer', 'Robert', 'Patricia', 'James', 'Linda', 'John', 'Barbara', 'David', 'Susan', 'Mark', 'Karen', 'Steven', 'Nancy', 'Daniel', 'Lisa', 'Paul', 'Margaret', 'Andrew', 'Betty', 'Joshua', 'Sandra', 'Kenneth', 'Dorothy', 'Brian', 'Nicholas', 'Helen', 'Ryan', 'Donna'];
const LAST = ['Whitfield', 'Hartman', 'Caldwell', 'Pemberton', 'Sinclair', 'Beaumont', 'Ashworth', 'Rutherford', 'Kingsley', 'Holloway', 'Thornton', 'Vance', 'Bellamy', 'Crawford', 'Donovan', 'Eastwood', 'Fairchild', 'Galloway', 'Harrington', 'Ingram', 'Jameson', 'Kensington', 'Lockhart', 'Montgomery', 'Northrop', 'Osborne', 'Prescott', 'Quincy', 'Radcliffe', 'Stratton'];
const STATES = ['CA', 'TX', 'NY', 'FL', 'PA', 'OH', 'IL', 'GA', 'NC', 'MI', 'NJ', 'VA', 'WA', 'AZ', 'MA', 'TN', 'IN', 'MO', 'MD', 'WI', 'CO', 'MN', 'SC', 'AL', 'LA', 'KY', 'OR', 'OK', 'CT', 'UT'];

const COMMITTEES = [
  { id: 'C001', name: 'Senate Banking', chamber: 'senate', sectors: ['Financials', 'Real Estate'] },
  { id: 'C002', name: 'House Energy & Commerce', chamber: 'house', sectors: ['Energy', 'Healthcare', 'Communication Services'] },
  { id: 'C003', name: 'Senate Armed Services', chamber: 'senate', sectors: ['Industrials', 'Aerospace & Defense', 'Information Technology'] },
  { id: 'C004', name: 'House Ways and Means', chamber: 'house', sectors: ['Financials', 'Healthcare', 'Consumer Discretionary'] },
  { id: 'C005', name: 'Senate Intelligence', chamber: 'senate', sectors: ['Information Technology', 'Aerospace & Defense', 'Communication Services'] },
  { id: 'C006', name: 'House Financial Services', chamber: 'house', sectors: ['Financials', 'Real Estate', 'Utilities'] },
  { id: 'C007', name: 'Senate Health, Education, Labor and Pensions', chamber: 'senate', sectors: ['Healthcare', 'Consumer Staples'] },
  { id: 'C008', name: 'House Judiciary', chamber: 'house', sectors: ['Information Technology', 'Communication Services'] },
];

const TICKERS = [
  { symbol: 'NVDA', company: 'NVIDIA Corporation', sector: 'Information Technology', gics: 'Semiconductors' },
  { symbol: 'MSFT', company: 'Microsoft Corporation', sector: 'Information Technology', gics: 'Systems Software' },
  { symbol: 'AAPL', company: 'Apple Inc.', sector: 'Information Technology', gics: 'Tech Hardware' },
  ...
  { symbol: 'NOC', company: 'Northrop Grumman', sector: 'Industrials', gics: 'Aerospace & Defense' },
  ...
  { symbol: 'UNH', company: 'UnitedHealth Group', sector: 'Healthcare', gics: 'Managed Health Care' },
  { symbol: 'TSLA', company: 'Tesla', sector: 'Consumer Discretionary', gics: 'Automobile Manufacturers' },
  { symbol: 'AMZN', company: 'Amazon.com', sector: 'Consumer Discretionary', gics: 'Internet Retail' },
  { symbol: 'GOOGL', company: 'Alphabet', sector: 'Communication Services', gics: 'Interactive Media' },
  { symbol: 'META', company: 'Meta Platforms', sector: 'Communication Services', gics: 'Interactive Media' },
  ...
  { symbol: 'PLTR', company: 'Palantir Technologies', sector: 'Information Technology', gics: 'Software' },
  { symbol: 'AMD', company: 'Advanced Micro Devices', sector: 'Information Technology', gics: 'Semiconductors' },
];
...
const members = Array.from({ length: 48 }, (_, i) => {
  ...
  return {
    ...
    sector_tilt: (() => {
      const sectors = ['Information Technology', 'Financials', 'Healthcare', 'Industrials', 'Energy', 'Consumer Discretionary', 'Communication Services'];
      const weights = sectors.map(() => rand());
      const total = weights.reduce((a, b) => a + b, 0);
      return sectors.map((s, k) => ({ sector: s, weight: weights[k] / total })).sort((a, b) => b.weight - a.weight);
    })(),
...
const flagsFor = (memberId: string, ticker: string) => {
  ...
  const f: any = {};
  if (rand() > 0.55) {
    f.jurisdiction_overlap = m.committees.slice(0, intBetween(1, m.committees.length)).map(cid => {
  ...
  if (rand() > 0.7) {
    f.hearing_proximity = {
      hearing_id: id('H'),
      topic: pick(['Semiconductor supply chain oversight', 'AI regulation framework', 'Defense procurement review', 'Banking sector stability', 'Healthcare pricing inquiry', 'Energy infrastructure resilience']),
  ...
  if (rand() > 0.85) {
    f.vote_trade_consistency = {
      ...
      bill_title: pick(['CHIPS Act Reauthorization', 'Defense Appropriations 2026', 'Healthcare Modernization Act', 'Banking Reform Act']),
...
const transactions = Array.from({ length: 320 }, (_, i) => {
  ...
  return {
    ...
    amount_min: bucket[0],
    amount_max: bucket[1],
    owner_type: pick(OWNER_TYPES as any),
...
const committees = COMMITTEES.map(c => {
  ...
  return {
    ...
    recent_cluster_trades: transactions
      .filter(t => t.flags.jurisdiction_overlap?.some((j: any) => j.committee_id === c.id))
      .slice(0, 8),
  };
});
...
const clusters = Array.from({ length: 14 }, (_, i) => {
  ...
  return {
    ...
    size_series: series(30, sz / 2, 0.4).map(p => ({ d: p.d, v: Math.max(1, Math.round(p.v + sz / 2)) })),
...
const tickers = TICKERS.map(t => {
  ...
  for (let i = 364; i >= 0; i--) {
    ...
    ohlc.push({ d: dateBack(i), o: +o.toFixed(2), h: +h.toFixed(2), l: +l.toFixed(2), c: +c.toFixed(2) });
    price = c;
  }
  return {
    symbol: t.symbol,
...
// === Leaderboards ===
const lbBuilder = (scoreOf: (m: any) => number, asc = false) => {
...
// === Alerts ===
const ALERT_KINDS = ['CLUSTER_THRESHOLD', 'CONTRACT_PROXIMITY', 'FOMC_BLACKOUT', 'WATCHLIST_MATCH', 'NEWS_CATALYST', 'INGESTION_HEALTH'] as const;
const alerts = Array.from({ length: 64 }, (_, i) => {
  const kind = pick(ALERT_KINDS as any) as typeof ALERT_KINDS[number];
  const m = pick(members);
  const t = pick(TICKERS);
...
const summary = {
  ...
  active_flagged_series: series(30, 8, 1.5).map(p => ({ d: p.d, v: Math.max(0, Math.round(p.v + 9)) })),
...
const signal_predictive = transactions
  ...
  .map((t, i) => ({
    ...
    signal_type: pick(['CONTRACT_LEAD', 'CLUSTER_FORMATION', 'LOBBYING_OVERLAY', 'HEARING_PRELUDE']),
...
const signal_reactive = transactions
  ...
  .map((t, i) => ({
    ...
    signal_type: pick(['POST_HEARING_TRADE', 'POST_VOTE_TRADE', 'NEWS_REACTION', 'BLACKOUT_VIOLATION']),
    member_id: t.member_id,
    member_name: t.member_name,
    ticker: t.ticker,
    score: t.signal_score ?? 0.5,
...
const committee_flow_top = ['Aerospace & Defense', 'Semiconductors', 'Pharmaceuticals', 'Diversified Banks', 'Integrated Oil & Gas']
  .map(sector => ({
    sector,
