...
export const Route = createFileRoute('/committees/')({
  head: () => ({
    meta: [
      ...
      { name: 'description', content: 'Congressional committees with jurisdictional sectors and member counts.' },
    ],
  }),
...
function CommitteesIndex() {
  ...
  return (
    <div className="space-y-3">
      <div>
        <h1 className="text-xs uppercase tracking-[0.18em] text-[var(--text-secondary)]">Committees</h1>
          ...
          <Link key={c.id} to="/committees/$id" params={{ id: c.id }}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="num mt-0.5 text-[10px] uppercase text-[var(--text-tertiary)]">{c.chamber} · {c.member_count} members</div>
              </div>
            </div>
            <p className="mt-2 text-xs text-[var(--text-secondary)] line-clamp-2">{c.jurisdiction_summary}</p>
